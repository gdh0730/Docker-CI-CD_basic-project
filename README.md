...
환경 일관성: 개발 환경과 배포 환경이 달라 발생하는 이슈 감소

빠른 배포: 애플리케이션 전체를 컨테이너로 패키징하여 배포 과정 단순화

확장성: 필요 시 손쉽게 컨테이너를 늘리거나 줄일 수 있음

Dockerfile 핵심

Base Image: 경량화를 위해 node:alpine 사용 (이미지 크기 최소화)

작업 디렉토리 설정: WORKDIR /app을 지정해 코드 가독성 향상
...
개발 생산성 향상: 여러 컨테이너(웹, DB, 캐시 등)를 단일 커맨드(docker-compose up -d)로 실행 가능

확장 및 로드 밸런싱: Scale 기능을 통해 컨테이너 개수를 쉽게 조절

환경 변수 관리: .env 파일을 통해 포트, 데이터베이스 URL 등 관리 간편
...
빠른 I/O 처리: 비동기 이벤트 기반 구조로 요청 처리에 유리

방대한 생태계: npm 라이브러리로 다양한 기능 손쉽게 구현

경량화: Docker와 결합 시 작은 메모리로도 웹 서버 구동 가능
...
사용 이유: 간단한 REST API 서버 또는 웹 서버 구축에 적합하며, Docker로 컨테이너화 시 핫 리로딩(개발 모드)과 경량화가 쉬움.

4️⃣ React 애플리케이션 실습

왜 React인가?

SPA 구조: 단일 페이지 내에서 빠른 화면 전환

컴포넌트화: UI 재사용성과 유지보수성 높음

생태계: 다양한 서드파티 라이브러리를 통한 기능 확장 용이
...
사용 이유: Dockerfile.dev를 통해 개발환경용 이미지를 별도로 관리, 개발 시 코드 변경사항이 즉시 반영.

5️⃣ GitHub Actions 기반 CI/CD

파이프라인 동작 원리

코드 푸시 시 이벤트 트리거: main 브랜치에 변경이 생기면 워크플로우 실행

자동 빌드 및 테스트: Docker 환경에서 빌드 후 단위/통합 테스트 수행

이미지 빌드 & 푸시: 성공 시 Docker Hub 등에 빌드된 이미지를 업로드

배포(옵션): 별도의 서버나 클라우드 환경에 이미지 배포/롤아웃 자동화
...
Docker 설치: Docker 공식 문서 참고

(옵션) Docker Compose 설치: Compose 설치 가이드

프로젝트 클론: git clone https://github.com/your-repo/docker-portfolio.git

폴더 진입: 예) cd nodejs-docker-app

이미지 빌드: docker build -t node-docker .

컨테이너 실행: docker run -p 3000:3000 node-docker

브라우저 열기: http://localhost:3000에서 결과 확인
...
개발 → Node.js & React 애플리케이션

Dockerization → Dockerfile(Docker Compose 포함)로 컨테이너화

CI/CD → GitHub Actions가 main 브랜치 푸시 시 빌드 & 테스트 수행, Docker Hub 배포

배포 → 서버에서 docker pull 후 docker run or docker-compose up
...
CI/CD의 중요성: 자동화된 배포 파이프라인으로 개발 속도와 품질 모두 향상
